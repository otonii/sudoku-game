...
export function validateRegionSize(level: Level): string[] {
  ...
  return level.regions.flatMap((region) => {
    ...
    if (region.cells.length < MIN_REGION_CELLS) {
      errors.push(
        `Região ${region.regionId} tem menos de ${MIN_REGION_CELLS} células.`,
      );
    }
...
export function validateRegionLinearity(level: Level): string[] {
  ...
  return level.regions.flatMap((region) => {
    ...
    for (const [row, count] of rowCounts) {
      if (count > limit) {
        errors.push(
          `Região ${region.regionId} ocupa ${count} células na linha ${row}.`,
        );
      }
    }
    ...
    for (const [column, count] of columnCounts) {
      if (count > limit) {
        errors.push(
          `Região ${region.regionId} ocupa ${count} células na coluna ${column}.`,
        );
      }
    }
...
export function isConcentratedStarterRegion(region: Region): boolean {
  ...
  return rows.size <= 2 || columns.size <= 2;
}
